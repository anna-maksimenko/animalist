...
function BOLD_mc_bootstrap(options) {
  ...
  document.addEventListener('DOMContentLoaded', function (event) {
    ...
    if (window.jQuery && $('#posterApp')[0]) {
      window.svelteLoadPickers = new Event('svelteLoadPickers')
      document.addEventListener('svelteLoadPickers', function(event){
        if (BOLDCURRENCY.converter.pickers){
          console.log(BOLDCURRENCY.converter.pickers)
          BOLDCURRENCY.converter.pickers.push(...initPickers());
          console.log(BOLDCURRENCY.converter.pickers)
          BOLDCURRENCY.converter.refresh()
          BOLDCURRENCY.converter.showAllInstances()
        } else {
